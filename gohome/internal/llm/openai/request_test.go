...
func TestBuildOpenAIBody_ReasoningEffort(t *testing.T) {
	req := common.Request{
		Model:           "o3",
		Messages:        []common.Message{{Role: common.RoleUser, Content: []common.Block{{Kind: common.BlockText, Text: "hi"}}}},
		MaxTokens:       1024,
	...
	raw, ok := body["reasoning_effort"]
	if !ok {
		t.Fatal("reasoning_effort missing from body")
	}
	var val string
	...
	if val != "high" {
		t.Errorf("reasoning_effort: got %q, want %q", val, "high")
	}
}

func TestBuildOpenAIBody_ReasoningEffortOmittedWhenEmpty(t *testing.T) {
	req := common.Request{
		Model:     "gpt-4o",
		Messages:  []common.Message{{Role: common.RoleUser, Content: []common.Block{{Kind: common.BlockText, Text: "hi"}}}},
		MaxTokens: 100,
