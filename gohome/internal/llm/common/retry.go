...
// DefaultBackoff is the delay schedule used between retry attempts.
var DefaultBackoff = []time.Duration{250 * time.Millisecond, time.Second, 2 * time.Second}

// DoWithRetry performs an HTTP request with retry on transient errors.
// It retries on connection errors and 5xx responses, but never on 4xx or
// context cancellation. The maximum number of attempts is len(backoff)+1.
// Failed 5xx bodies are drained and closed before each retry.
